...
const OptimizationTools: React.FC<Props> = ({ portfolio, onUpdatePortfolio }) => {
    ...
    const handleExecute = () => {
        ...
        const newAssets = portfolio.assets.map(asset => {
            const proposal = result.proposedAllocation.find(p => p.ticker === asset.ticker);
            return {
                ...asset,
                allocation: proposal ? proposal.weight * 100 : 0
            };
        });
                    ...
                    <div className="bg-nexus-800 border border-nexus-700 rounded-2xl p-6 shadow-lg">
                        ...
                        <div className="mb-6">
                            <div className="flex justify-between mb-2">
                                ...
                                <span className={`text-xs font-bold px-2 py-0.5 rounded ${riskLevel === RiskLevel.LOW ? 'bg-emerald-500/20 text-emerald-400' :
                            ...
                            <div className="space-y-3">
                                {(Object.keys(STRATEGY_DESCRIPTIONS) as Array<keyof typeof STRATEGY_DESCRIPTIONS>).map((type) => (
                                    <div
                                        ...
                                        className={`p-3 rounded-lg border cursor-pointer transition-all ${strategyType === type
                    ...
                    {!isGenerating && result && (
                        <div className="space-y-6">
                            ...
                            <button
                                ...
                                className={`w-full py-3 font-bold rounded-xl transition-colors shadow-lg ${executed
                                        ? 'bg-emerald-700 text-emerald-100 shadow-none'
                                        : 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-900/20'
                                    }`}
                            >
                                {executed ? 'Rebalancing Executed!' : 'Execute Rebalancing (Simulated)'}
                            </button>
                        </div>
                    )}
                </div>
